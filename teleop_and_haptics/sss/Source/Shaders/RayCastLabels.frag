...
// --- Label Field Surfaces ------------------------------------------------- //

float labelValue(vec3 p)
{
    // this version multiplies the sample by the mask value at same location
    return texture3D(g_volumeLabels, p).a * texture3D(g_volumeMask, p).r;
}

// assumes p is in texture space, returns gradient magnitude in the w channel
vec4 labelGradient(vec3 p)
{
    float gx = labelValue(p+dx) - labelValue(p-dx);
    float gy = labelValue(p+dy) - labelValue(p-dy);
    float gz = labelValue(p+dz) - labelValue(p-dz);
    vec3 g = vec3(gx, gy, gz);
    float l = length(g);
    return vec4(g / (l+0.001), l);
}

vec4 label(inout vec4 ray, vec4 rstep, vec3 rdir, vec4 dist)
{
    float index = texture3D(g_volumeLabels, ray.xyz).r;
    vec4 colour = texture1D(g_transferLabels, index);

    // refine the hit one the label surface
    vec4 bstep = rstep * 0.5;
    ray -= bstep;
    for (int i = 0; i < 8; ++i) {
        float sample = labelValue(ray.xyz);
        bstep *= 0.5;
        float f = step(0.5, sample) * -2.0 + 1.0;
        ray += bstep * f;
    }
    
    // shade the surface and return the colour
    vec4 lit = vec4(0.0, 0.0, 0.0, 0.0);
    if (ray.w < dist.t) {
        vec3 normal = labelGradient(ray.xyz).xyz;
        lit = phong(normal, rdir, rdir, colour, 0.2, 0.6, 0.3, 20.0);
    }
...
void main(void)
{
    ...
    while (ray.w < dist.t && sum.a < k_opacityLimit)
    {
        ...
        {
            sum += (1.0 - sum.a) * label(ray, rayStep, rayDirection, dist);
            break;
        }
        ...
        do {
            ...
            sum += (1.0 - sum.a) * lit;
        } while (inside && !labelled && sum.a < k_opacityLimit);
