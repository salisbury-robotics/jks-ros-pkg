...
int main(int argc, char* argv[])
{
    ...
    // load an object file
	fileload = tooth->loadFromFile("./resources/models/tooth/tooth.3ds");
	if (!fileload)
	{
		#if defined(_WIN32)
    ...
    // make the outside of the tooth rendered in wireframe9
	...
	fileload = drill->loadFromFile("./resources/models/drill/drill.3ds");
	if (!fileload)
	{
		#if defined(_WIN32)
...
void updateHaptics(void)
{
    ...
    while(simulationRunning)
    {
        ...
        if (button == 0)
        {
            ...
            tool->setShow(true, true);
            drill->setShow(true, true);
        }
        else
        {
            tool->setShow(false, true);
            drill->setShow(false, true);
