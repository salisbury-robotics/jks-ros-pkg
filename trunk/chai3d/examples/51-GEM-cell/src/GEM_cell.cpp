...
int main(int argc, char* argv[])
{
    ...
    const double FORCE_REFERENCE = 0.5;
    ...
    device->m_material.m_ambient.set(1.0, 0.4, 0.4, 0.7);
    device->m_material.m_diffuse.set(1.0, 0.7, 0.7, 0.7);
    device->m_material.m_specular.set(1.0, 1.0, 1.0, 0.7);
    device->m_material.setShininess(100);
    device->enableTransparency(true);
    ...
    defObject = new cDefMesh(world);
    defWorld->m_defMeshes.push_front(defObject);
	fileload = defObject->loadFromFile("./resources/models/cell/cell.obj");
	if (!fileload)
	{
		#if defined(_WIN32)
    ...
    default_node_radius        = 0.04;//3;
    default_node_kDampingPos   = 0.4;
    default_node_kDampingRot   = 0.1;
    default_node_mass          = 0.06;  // [kg]
    default_node_showFrame     = false;
    default_node_color.set(0.6, 0.6, 0.0);
    default_node_useGravity      = false;
    default_node_gravity.set(0.00, 0.00, -1.00);
    radius = default_node_radius;

    // setup default values for links
    default_link_kSpringElongation = 100.0; // [N/m]
    default_link_kSpringFlexion    = 0.5;   // [Nm/RAD]
    default_link_kSpringTorsion    = 0.1;   // [Nm/RAD]
    default_link_color.set(0.2, 0.2, 1.0);
    ...
    for(i = defObject->m_nodes.begin(); i != defObject->m_nodes.end(); ++i)
    {
        if (counter1 <= num)
        {
            ...
            {
                cDefNode* nextItem = *i;
                cDefNode* newNode = new cDefNode();
                newNode->m_fixed = true;
                newNode->m_pos = nextItem->m_pos;
                cDefLink* newLink = new cDefLink(nextItem, newNode); defObject->m_links.push_front(newLink);
...
void updateHaptics(void)
{
    ...
    while(simulationRunning)
    {
        ...
        for(i = defObject->m_nodes.begin(); i != defObject->m_nodes.end(); ++i)
        {
            ...
            cVector3d forcec = computeForce(pos, 0, nodePos, deviceRadius+nextItem->m_radius, 1);
            force.add(forcec);

            if (force.length() > 0)
            {
                nextItem->setExternalForce(-0.2*force);
            }
        }

        // update dynamics
        defWorld->updateDynamics(1.0/500.0);
...
void BuildDynamicModel()
{
    ...
    //-----------------------------------
    newNode = new cDefNode(); defObject->m_nodes.push_front(newNode); 
    newNode->m_pos.set(0.296,0.280,-0.213); 
    //-----------------------------------
    prevNode = newNode; newNode = new cDefNode(); defObject->m_nodes.push_front(newNode); 
    newNode->m_pos.set(0.286,0.213,-0.254); 
    newLink = new cDefLink(prevNode, newNode); defObject->m_links.push_front(newLink); 
    //-----------------------------------
    prevNode = newNode; newNode = new cDefNode(); defObject->m_nodes.push_front(newNode); 
