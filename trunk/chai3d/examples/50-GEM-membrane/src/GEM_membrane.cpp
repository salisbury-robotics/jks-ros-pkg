...
int main(int argc, char* argv[])
{
	...
	fileload = defObject->loadFromFile("./resources/models/box/box.3ds");
	if (!fileload)
	{
		#if defined(_WIN32)
	...
	fileload = texture->loadFromFile("./resources/images/shadow.bmp");
	if (!fileload)
	{
		#if defined(_WIN32)
    ...
    defObject->useTexture(true, true);

    // setup default values for nodes
    default_node_radius        = 0.05;
    default_node_kDampingPos   = 0.4;
    default_node_kDampingRot   = 0.1;
    default_node_mass          = 0.05;  // [kg]
    default_node_showFrame     = true;
    default_node_color.set(1.0, 0.6, 0.6);
    default_node_useGravity    = true;
    ...
    for (int y=0; y<9; y++)
    {
        for (int x=0; x<9; x++)
        {
            cDefLink* newLinkX0 = new cDefLink(nodes[x+0][y+0], nodes[x+1][y+0]);
            cDefLink* newLinkX1 = new cDefLink(nodes[x+0][y+1], nodes[x+1][y+1]);
            cDefLink* newLinkY0 = new cDefLink(nodes[x+0][y+0], nodes[x+0][y+1]);
            cDefLink* newLinkY1 = new cDefLink(nodes[x+1][y+0], nodes[x+1][y+1]);
    ...
    while(simulationRunning)
    {
        ...
        for (int y=0; y<10; y++)
        {
            for (int x=0; x<10; x++)
            {
               ...
               nodes[x][y]->setExternalForce(cNegate(f));
               force.add(f);
            }
        }
